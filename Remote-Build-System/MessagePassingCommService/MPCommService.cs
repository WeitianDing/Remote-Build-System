...
namespace MessagePassingComm
{
    ...
    public class Receiver : IMessagePassingComm
    {
        ...
        public bool openFileForWrite(string name,string targetStorage)
        {
            try
            {
                string writePath = Path.Combine(targetStorage, name);
    ...
    public class Sender
    {
        ...
        public bool postFile(string fileName, string thisStorage,string targetStorage)
        {
            FileStream fs = null;
            long bytesRemaining;
            ...
            try
            {
                string path = Path.Combine(thisStorage, fileName);
                fs = File.OpenRead(path);
                bytesRemaining = fs.Length;
                channel.openFileForWrite(fileName, targetStorage);
                while (true)
                {
